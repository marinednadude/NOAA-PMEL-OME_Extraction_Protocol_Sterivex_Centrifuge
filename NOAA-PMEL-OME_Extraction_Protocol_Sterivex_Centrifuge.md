...
5. Turn on the heatshaker to 56°C. If the Buffer ATL has percipitate, place in heatshaker. Allow the buffer to heat until all percipitate is gone (~15 minutes), swirl as needed.
6. Aliquot ~25 mL of 100% EtOH into a 50 mL falcon tube and store at -20°C freezer for later use.
7. Ensure 100% molecular grade EtOH has been added to Buffer AW1 and AW2 according to the Qiagen DNeasy Blood and Tissue Kit manufactorer instructions. 
...
7. Add 720 µL Buffer ATL (pre-heated if percipitate was present) and 80 µL Proteinase K to each sterivex with the 1000 μL and 200 μL pipettes, respectively. Pipet solution between the outside of the filter and the capsule walls. Close with a new inlet cap. Handshake vigorously and/or vortex sideways.
...
14. Pipet the mixture (max 640 µL) into a spin columns in a 2 mL collection tube provided by the DNeasy Blood and Tissue kit. 
15. Spin in the centrifuge at 6000 ∗ g for 1 min. Discard flow through. Tap collection tubes on a kimwipe to remove droplets before replacing column in collection tube.
16. Repeat steps 14-15 until the full samples volume is filtered through each spin column (takes 4-5x).
...
17. Discard flow-through and collection tube. Transfer spin column to a new, labeled 1.5 mL tube with the cap open.
